...
pub(crate) struct HandshakeConfig {
    ...
    // MIRAGE FINGERPRINT PATCH: when true, the client ClientHello advertises
    // Chrome/libwebrtc's exact DTLS cipher-suite + signature-algorithm lists and
    // extension shape (see flight1::chrome_client_hello_extensions) instead of
    // echoing local_cipher_suites/local_signature_schemes. Set automatically for
    // the default ECDHE (non-PSK, no explicitly-configured suites) case, which
    // is exactly how the high-level `webrtc` crate configures DTLS.
    pub(crate) offer_chrome_fingerprint: bool,
...
impl Default for HandshakeConfig {
    fn default() -> Self {
        HandshakeConfig {
            ...
            initial_epoch: 0,
            offer_chrome_fingerprint: false,
        }
    }
}
