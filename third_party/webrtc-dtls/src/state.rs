...
pub struct State {
    ...
    // MIRAGE FINGERPRINT PATCH: an INDEPENDENT CSPRNG seed for the ClientHello
    // extension permutation (flight1). It is generated once and reused by the
    // flight3 cookie-retransmit so both ClientHellos share the same order, but
    // — unlike seeding from client_random — it is NOT marshaled to the wire, so
    // a censor cannot recompute `order = f(client_random)` and detect us. All
    // zero means "not yet generated".
    pub(crate) ch_ext_perm_seed: [u8; 32],
