...
// MIRAGE FINGERPRINT PATCH: the exact `signature_algorithms` list a Chrome/
// libwebrtc DTLS 1.2 ClientHello (and its CertificateRequest) carries, as raw
// RFC 8446 §4.2.3 code points, in order (9 entries, NO GREASE). This is the raw
// BoringSSL `kVerifySignatureAlgorithms` default — note it INCLUDES the trailing
// `rsa_pkcs1_sha1` (0x0201), which the Chrome *HTTPS* stack strips but the
// WebRTC DTLS path does not (libwebrtc never calls SSL_set1_sigalgs), and it
// EXCLUDES ed25519 (0x0807, off by default in BoringSSL's verify prefs).
// Verified against real captures (covert-dtls, FOCI 2025) + BoringSSL source.
// These are advertised for wire parity only; actual signing/verification still
// uses the {hash,sig} schemes in local_signature_schemes (ECDSA-P256 for our
// self-signed WebRTC identity), so no PSS crypto is required.
pub(crate) fn chrome_dtls_signature_schemes() -> Vec<u16> {
    vec![
        SignatureScheme::EcdsaWithP256AndSha256 as u16, // 0x0403
        SignatureScheme::PssWithSha256 as u16,          // 0x0804
        SignatureScheme::Pkcs1WithSha256 as u16,        // 0x0401
        SignatureScheme::EcdsaWithP384AndSha384 as u16, // 0x0503
        SignatureScheme::PssWithSha384 as u16,          // 0x0805
        SignatureScheme::Pkcs1WithSha384 as u16,        // 0x0501
        SignatureScheme::PssWithSha512 as u16,          // 0x0806
        SignatureScheme::Pkcs1WithSha512 as u16,        // 0x0601
        SignatureScheme::Pkcs1WithSha1 as u16,          // 0x0201
    ]
}
