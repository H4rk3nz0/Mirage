...
impl DTLSConn {
    pub async fn new(
        ...
        // MIRAGE FINGERPRINT PATCH: advertise Chrome's DTLS fingerprint only in
        // the default ECDHE case — the caller left cipher_suites unset and isn't
        // using PSK. Callers that pin specific suites (or use PSK) get the
        // upstream behavior (offer exactly what was configured), so the crate's
        // own negotiation/PSK semantics and tests are unchanged.
        let offer_chrome_fingerprint = config.cipher_suites.is_empty() && config.psk.is_none();
