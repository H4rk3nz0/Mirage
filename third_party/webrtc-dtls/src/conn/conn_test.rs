...
async fn test_cipher_suite_configuration() -> Result<()> {
    ...
    let tests = vec![
        ...
        (
            // MIRAGE FINGERPRINT PATCH: full DTLS handshake negotiated on
            // ChaCha20-Poly1305 (RFC 7905). Handshake completion here means the
            // encrypted Finished round-tripped through our new AEAD, exercising
            // the implicit-nonce record path end-to-end (not just the primitive).
            "Valid CipherSuites ChaCha20-Poly1305 specified",
            vec![CipherSuiteId::Tls_Ecdhe_Ecdsa_With_Chacha20_Poly1305_Sha256],
            vec![CipherSuiteId::Tls_Ecdhe_Ecdsa_With_Chacha20_Poly1305_Sha256],
            None,
            None,
            Some(CipherSuiteId::Tls_Ecdhe_Ecdsa_With_Chacha20_Poly1305_Sha256),
        ),
    ];
...
async fn test_server_timeout() -> Result<()> {
    ...
    let extensions = vec![
        Extension::SupportedSignatureAlgorithms(ExtensionSupportedSignatureAlgorithms {
            // MIRAGE: raw u16 SignatureScheme codes (ecdsa/rsa-pkcs1 sha256/384/512).
            signature_schemes: vec![0x0403, 0x0503, 0x0603, 0x0401, 0x0501, 0x0601],
        }),
