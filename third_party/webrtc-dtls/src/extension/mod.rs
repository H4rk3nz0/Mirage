...
pub enum ExtensionValue {
    ...
    // MIRAGE FINGERPRINT PATCH: a RFC 8701 GREASE extension type. Registering
    // it as "known" means our own answerer (which errors on truly-unknown
    // extension types) parses+ignores it instead of failing the handshake.
    Grease = 0x2a2a,
    Unsupported = 0xffff,
}
...
impl From<u16> for ExtensionValue {
    fn from(val: u16) -> Self {
        match val {
            ...
            // Any RFC 8701 GREASE value (both bytes equal, low nibble 0xA)
            // maps to the GREASE extension so it is parsed + ignored.
            v if (v >> 8) == (v & 0xff) && (v & 0x0f) == 0x0a => ExtensionValue::Grease,
...
impl Extension {
    pub fn extension_value(&self) -> ExtensionValue {
        match self {
            ...
            Extension::Grease(ext) => ext.extension_value(),
        }
    }
    ...
    pub fn marshal<W: Write>(&self, writer: &mut W) -> Result<()> {
        ...
        match self {
            ...
            Extension::Grease(ext) => ext.marshal(writer),
        }
    }
    ...
    pub fn unmarshal<R: Read>(reader: &mut R) -> Result<Self> {
        ...
        match extension_value {
            ...
            ExtensionValue::Grease => Ok(Extension::Grease(ExtensionGrease::unmarshal(reader)?)),
