...
impl HandshakeCache {
    ...
    pub(crate) async fn session_hash(
        ...
        // MIRAGE FINGERPRINT PATCH: the transcript/session hash must use the
        // negotiated cipher-suite's PRF hash — SHA384 for AES-256-GCM-SHA384 —
        // not a hardcoded SHA256. The two digest states are distinct types, so
        // branch and finalize within each arm.
        let result = match hf {
