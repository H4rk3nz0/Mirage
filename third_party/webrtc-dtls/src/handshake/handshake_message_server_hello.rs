...
impl HandshakeMessageServerHello {
    ...
    pub fn unmarshal<R: Read>(reader: &mut R) -> Result<Self> {
        ...
        while offset < extension_buffer_len {
            // Every extension is prefixed by a 4-byte header (2-byte type +
            // 2-byte length). A malformed or truncated tail must not index out
            // of bounds -- that would be a pre-auth remote panic. Stop parsing
            // extensions if fewer than 4 header bytes remain.
