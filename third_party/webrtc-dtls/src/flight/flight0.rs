            ...
            for extension in &client_hello.extensions {
                match extension {
                    Extension::SupportedEllipticCurves(e) => {
                        ...
                        // MIRAGE FINGERPRINT PATCH: skip GREASE/unsupported
                        // curves (RFC 8701) — a real server picks the first
                        // curve it actually supports, not blindly [0].
                        state.named_curve = e
                            .elliptic_curves
                            .iter()
                            .copied()
                            .find(|c| *c != NamedCurve::Unsupported && *c != NamedCurve::Grease)
