...
use super::flight1::client_hello_body;
use super::flight5::*;
use super::*;
...
// MIRAGE FINGERPRINT PATCH: the ClientHello extension/cipher block is now built
// by flight1::client_hello_body (shared with flight1), so flight3 no longer
// constructs the individual extension types itself. use_srtp stays — flight3's
// parse() still matches SRTP profiles on the server's response.
...
impl Flight for Flight3 {
    ...
    async fn generate(
        ...
        // MIRAGE FINGERPRINT PATCH (keep in sync with flight1.rs): identical
        // ClientHello. Seeded from the SAME client_random flight1 used (unchanged
        // across the cookie retransmit), so this cookie'd ClientHello is
        // byte-identical to the initial one bar the cookie.
        let (extensions, cipher_suites) =
            client_hello_body(cfg, &state.ch_ext_perm_seed);

        Ok(vec![Packet {
            record: RecordLayer::new(
                // MIRAGE FINGERPRINT PATCH (see flight1.rs): the cookie-retransmit
                // ClientHello is still pre-negotiation, so its RECORD version is
                // DTLS 1.0 (0xfeff) too — byte-identical to the initial one bar
                // the cookie. The body client_version stays 0xfefd.
                PROTOCOL_VERSION1_0,
                0,
