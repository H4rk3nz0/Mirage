...
impl FragmentBuffer {
    ...
    fn size(&self) -> usize {
        // Count each fragment's handshake-header overhead in addition to its
        // data, so that FRAGMENT_BUFFER_MAX_SIZE also bounds the *number* of
        // cached fragments. Without this, a peer could flood zero-length
        // handshake fragments (each contributing 0 data bytes) and grow the
        // cache without ever tripping the overflow guard.
        self.cache
            .values()
            .map(|fragment| {
                fragment
                    .iter()
                    .map(|f| f.data.len() + HANDSHAKE_HEADER_LENGTH)
                    .sum::<usize>()
            })
            .sum()
    }
...
fn append_message(target_offset: u32, frags: &[Fragment], raw_message: &mut Vec<u8>) -> bool {
    // Walk the fragment chain iteratively rather than recursively. The original
    // implementation recursed once per fragment in the chain, so a peer that
    // sends many small consecutive fragments (bounded only by the 2 MB
    // FRAGMENT_BUFFER_MAX_SIZE, i.e. potentially hundreds of thousands of
    // frames) could drive the recursion deep enough to overflow the stack --
    // a pre-auth remote DoS during the DTLS handshake. Iterating keeps the
    // reassembly semantics identical while using O(1) stack.
    //
    // Index fragments by offset once (first occurrence wins, matching the
    // original linear-scan order) so chain-walking is O(n) instead of the
    // original O(n^2) -- otherwise a maxed-out fragment buffer would still be a
    // CPU DoS.
    let mut by_offset: std::collections::HashMap<u32, &Fragment> =
        std::collections::HashMap::with_capacity(frags.len());
    for f in frags {
        by_offset
            .entry(f.handshake_header.fragment_offset)
            .or_insert(f);
    }
    ...
    loop {
        let Some(&f) = by_offset.get(&offset) else {
            return false;
        };
        ...
        // NB: Order matches the original recursion -- only continue the chain
        // when this fragment neither completes the message nor is zero-length.
        if fragment_end == f.handshake_header.length || f.handshake_header.fragment_length == 0 {
            break;
        }

        offset = fragment_end;

        // Offsets strictly increase whenever we continue (fragment_length != 0
        // above), so the chain can never revisit a fragment; this is a
        // defensive backstop against a malformed frags slice.
        if chain.len() > frags.len() {
            return false;
        }
    }

    // Assemble in ascending-offset order, then prepend onto any existing
    // message content -- matches the recursive version, which prepended each
    // fragment's data as the stack unwound.
    let mut message = Vec::with_capacity(chain.iter().map(|f| f.data.len()).sum());
    for f in &chain {
